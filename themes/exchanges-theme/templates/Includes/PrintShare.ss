<p class="pull-right printShare">
	...
	<% end_if %>
	<% if ICSLink %>
		| <a href="$ICSLink" title="<% _t('CalendarEvent.ADD','Add to Calendar') %>">Add this to Calendar</a>
	<% end_if %>
	<% if Link(rss) %>
		| <a href="$Link(rss)"><% _t('SUBSCRIBE','Calendar RSS Feed') %></a>
	<% else_if $Parent.Link(rss) %>
